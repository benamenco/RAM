...
core.Taxa<-function(data, is.OTU=FALSE, rank="g", 
  ...
  for ( i in 1:length(data) ) {
   ...
   for ( j in 1:length(fac.levels) ) {
     ...
     # presence or absence
     tax.all.fac.pa <- decostand(tax.all.fac, "pa")
...
group.OTU <- function(otu, rank="g", otuIDs="", meta, 
 ...
 # }
  ...
  otu.sel.fac.m<-melt(cbind(otu.sel.fac, 
                     Sample=rownames(otu.sel.fac)), 
  ...
  if(boxplot) {
    # boxplot
    otu.p.sel.fac.m<-melt(cbind(otu.p.sel.fac, 
...
group.venn <- function(vectors, cat.cex=1.5, cex=1,
 ...
 if ( !is.null(cat.pos) && !is.null(cat.dist) ) { 
  v <- venn.diagram(vectors, fill = fill, alpha = alpha, 
  ...
  v <- venn.diagram(vectors, fill = fill, alpha = alpha, 
                cat.fontface = "bold", 
                cat.dist= cat.dist, cat.cex = cat.cex, cex=cex,
                filename=NULL)
 } else {
     v <- venn.diagram(vectors, fill = fill, alpha = alpha, 
