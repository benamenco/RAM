group.Taxa.box <- function(data, is.OTU=TRUE, rank="g", 
                      ...
                      cex.y=5, cex.x=5, RAM.theme=NULL, 
                      col.pal=NULL, main="", 
  ...
  for (i in 1:length(data)) {
      ...
      tax.sel <- tax[, which(names(tax) %in% taxa)]
  ...
  for (i in 1:length(tax.list)) {
    ...
    if ( identical(rownames(tax.sel), rownames(meta)) ) {
        tax.sel.p  <-  decostand(tax.sel, "total", MARGIN=2)
 ...
 # boxplot 
 p <- ggplot(all.taxa, aes_string(x=rank_name,
  ...
  if ( is.null(RAM.theme) ) {     
     p <- p + theme(legend.key.size=unit(6,"mm"), 
              ...
              axis.text.y=element_text(size=cex.y),
              axis.text.x=element_text(size=cex.x),
...
#      stop("package 'grid' is required for this function")
 # }
  ...
  # http://goo.gl/JxgZ9u
  if ( is.null(bar.width) ) {
    p <- ggplot(all.taxa, aes_string(x="Sample", y="RA", 
              fill=rank_name)) + 
 ...
 # check whether data is list
  ...
  for (i in 1:length(data)) {
     ...
     if( length(which(colSums(tax.pa) == x ))==0 ) {
          stop("NO taxa are shared by all samples")
     } else {

        num.tax.one.sample <- dim(tax.pa[ ,colSums(tax.pa) == 1])[2]
        num.tax.mult.sample <- dim(tax.pa[ ,colSums(tax.pa) > 1])[2] 
        ### test this
        num.tax.all.sample <- dim(tax.pa[ ,colSums(tax.pa) == x])[2]
...
.cols <- function(cols.needed, col.pal=NULL) {
   if ( is.null(col.pal) ) {    
      if ( cols.needed <= 2 ) {
          ...
          cols <- brewer.pal(cols.needed, "Set3")
      } else {
...
.group.rank<-function(data, is.OTU=TRUE, meta=meta, meta.factor="",
  ...
  if ( is.null(taxa) ) {
      ...
      if( !any(names(tax.factor) %in% taxa) ) {
         ...
         tax.factor.sel <- tax.factor[, which(names(tax.factor) %in% taxa)]
